    ...
    def test_applicability_filters_groups(self):
        flow = _base_flow_dict(
            optional_groups=[
                _og(
                    "R01",
                    [_step("ret1", "return", depends_on=["ach_pull"])],
                    applicable_when={"requires_step_match": [{"payment_type": "ach"}]},
                ),
                _og(
                    "Wire Return",
                    [_step("wret", "return", depends_on=["settle"])],
    ...
    def test_weighted_exclusion_group(self):
        ...
        flow = _base_flow_dict(
            optional_groups=[
                _og(
                    "A",
                    [_step("sa", "return", depends_on=["ach_pull"])],
                    exclusion_group="method",
                    weight=999.0,
                ),
                _og(
                    "B",
                    [_step("sb", "return", depends_on=["settle"])],
                    exclusion_group="method",
                    weight=0.001,
                ),
            ]
...
class TestFlattenPosition:
    ...
    def test_after_with_anchor(self):
        flow = _base_flow_dict(
            optional_groups=[
                _og(
                    "OG",
                    [_step("og1", "return", depends_on=["ach_pull"])],
                    position="after",
                    insert_after="ach_pull",
                ),
            ]
        )
        flatten_optional_groups(flow)
    ...
    def test_before_with_anchor(self):
        flow = _base_flow_dict(
            optional_groups=[
                _og(
                    "OG",
                    [_step("before_settle", "expected_payment")],
                    position="before",
                    insert_after="settle",
                ),
            ]
    ...
    def test_replace_missing_anchor_falls_back_to_append(self):
        flow = _base_flow_dict(
            optional_groups=[
                _og("OG", [_step("og1", "return")], position="replace", insert_after="nonexistent"),
            ]
        )
        flatten_optional_groups(flow)
        ids = [s["step_id"] for s in flow["steps"]]
    ...
    def test_activated_filter_respects_position(self):
        flow = _base_flow_dict(
            optional_groups=[
                ...
                _og("Inactive", [_step("og2", "return", depends_on=["settle"])], position="before"),
            ]
        )
    ...
    def test_position_after_anchor_orders_og_steps_correctly(self):
        ir = self._compile(
            [
                _og(
                    "Mid",
                    [_step("mid1", "return", depends_on=["ach_pull"])],
                    position="after",
                    insert_after="ach_pull",
                ),
            ]
