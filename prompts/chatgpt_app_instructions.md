...
**Top level when authoring:** static/bootstrap — `connections`, `legal_entities`, `counterparties`, `internal_accounts`, `external_accounts`, ledgers/LAs/categories/VAs as needed, per-flow `instance_resources`, plus **`funds_flows`**. Do **not** hand-write lifecycle root arrays (`payment_orders`, `incoming_payment_details`, …, **`verify_external_accounts`**, **`complete_verifications`**, **`archive_resources`**) in generated JSON — express verification, completion, and archive as **`funds_flows[].steps`** (and `optional_groups`). The **compiler** emits those sections on the flat `DataLoaderConfig` after compile (same pattern as `payment_orders`; they are real schema fields on merged output, not “step-only with no root arrays”). See **`decision_rubrics.md`** § Root JSON and *Flat vs authoring*.
